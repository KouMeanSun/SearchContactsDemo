...
{
    struct PASEArray SearchDataPASEArray;                           //SearchData缓存
    struct PASEArray iCachedHits[KCachedHitNum];                //汉字的拼音的首字母缓存
    struct SearchData *iCurSeachData;                       //当前输入转换的搜索串
    struct PASEArray iMatchTrace;                               //当前匹配路径
 ...
 * 搜索数据
 * aText: 搜索串
 * aSearchedPASEArray 搜索域，输入NULL时搜索tree中的联系人
 * aNameMatchPASEArray: 名字搜索结果
 * aPhoneMatchPASEArray：号码搜索结果
 * sortByMatchValue：按搜索位置排序
 */
void Tree_Search(SearchTree* tree, u2char* aText, PASEArray* aSearchedPASEArray,PASEArray* aNameMatchPASEArray, PASEArray* aPhoneMatchPASEArray);
 ...
 */
BOOL Tree_GetPinYin(SearchTree* tree,NSInteger aID, u2char* aText, PASEArray* iMatchPosInPinYin);

/*
 * 获取号码匹配的号码、匹配位置
 */
BOOL Tree_GetPhoneNum(SearchTree* tree,NSInteger aID, u2char* aText, PASEArray* iMatchPosInPhoneNum);
...
void AddToCachedHitSingle(SearchTree* tree,SearchData *aData, PASEArray* aCachePASEArray);

//kmp匹配 适用于查找连续字串——号码
BOOL IsMatchByKmp(const u2char* aText,const u2char* wordInput,PASEArray* iMatchPosInPinYin);

//aID 的索引index  < 0为不存在
NSInteger FindSearchDataIndex(PASEArray* ptr,NSInteger aID,SearchData** data);

/*
 * < 0 已存在
 * > 0 需插入位置
 */
NSInteger FindSearchDataInsertIndex(PASEArray* ptr,NSInteger aID);
...
BOOL SearchCachedHit(SearchTree* tree, u2char word, PASEArray **aHits);
