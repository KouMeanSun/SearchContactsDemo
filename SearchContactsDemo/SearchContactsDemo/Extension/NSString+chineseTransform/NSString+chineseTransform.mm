...
+ (NSMutableAttributedString *)lightStringWithSearchResultName:(NSString *)searchResultName matchPASEArray:(NSArray *)matchPASEArray inputString:(NSString *)inputString lightedColor:(UIColor *)lightedColor{
    ...
    NSUInteger matchCount = matchPASEArray.count;
    for (int i = 0; i < matchCount; i ++) {
        NSNumber * compareNum = matchPASEArray[i];
        int j = 0;
        [self compare:&j searhRestultName:searchResultName compareNum:compareNum matchPASEArray:matchPASEArray];
        j = j >= 1 ? j - 1 : j;
...
+ (void)compare:(int *)i searhRestultName:(NSString *)searchResultName compareNum :(NSNumber *)compareNum  matchPASEArray:(NSArray *)matchPASEArray{
    ...
    if ([NSString needLightChar:[searchResultName substringWithRange:NSMakeRange(0, (*i))] compareValue:compareNum.intValue]) {
            ...
            *i = *(i) + 1;
            [self compare:i searhRestultName:searchResultName compareNum:compareNum matchPASEArray :matchPASEArray];
        }
